...
export class AddGroupPage extends ImagePage {
  ...
  constructor(
    ...
    this.groupObject = {
      image: 'assets/img/empty_image.png',
      members: []
    };
  }
  ...
  saveGroup() {
    ...
    //LLAMAR API
    if (0 === message.length) {
      this.groupProvider.saveGroup({ 
          group: {
              name: this.groupObject.name,
              is_open: this.groupObject.is_open
          },
          users: this.groupObject.members
        })
        .subscribe((response) => {
          if (this.groupObject.image) {
            this.groupProvider.uploadGroupImage(this.groupObject.image, response.group.id)
              .then(() => {
